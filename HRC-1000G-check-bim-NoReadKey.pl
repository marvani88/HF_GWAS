...
GetOptions
 ...
 "s|splitchr"	 => \$splchrflag  # makes split by chromosome VCF files in bash script
  );
...
sub getwidth
 {
 #removed backticks as only works on Unix based machines
 #my $output = `stty size`;
 #my @rowcols = split(/\s/, $output);
 #my $cols = $rowcols[1];
 
 #removed terminal size for running in a non-interactive session
 #my @winsize = &GetTerminalSize(\*STDOUT);
 #my ($cols, $rows, $xpix, $ypix) = @winsize;
 
 #set cols to 80 as default
 my $cols = 80;
 #if (!$cols)
 # {
 # $cols = 80;
